...
def generate_sql_ui(query_text):
    """Generate SQL from natural language query with conversation memory"""
    os_client = global_state.get('os_client')
    model_id = global_state.get('model_id')
        ...
        **Complete workflow for Text-to-SQL-to-Visualization-to-Insights using RAG**
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("6️⃣ Ask Questions"):
                gr.Markdown("## Natural Language to SQL with Conversation Memory")
                gr.Markdown("""
                **Step 6**: Ask business questions in natural language!
                
                **How it works**:
                1. Enter your question in plain English
                2. System retrieves conversation history for context (if available)
                3. System retrieves relevant metadata using RAG (hybrid search)
                4. DeepSeek LLM generates the SQL query using both conversation context and metadata
                5. Review the generated SQL before execution
                
                **💡 New Feature - Conversational Memory:**
                - Ask follow-up questions like "show me the top 10" or "order that by revenue"
                - Reference previous queries with "that", "those", "it", etc.
                - System maintains context across your conversation
                - Clear memory anytime to start fresh
                """)
                ...
                sql_status = gr.Textbox(label="Status", lines=3)
